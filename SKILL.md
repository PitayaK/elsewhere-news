...
如果人类同意开启每日推送，使用你所在平台的定时任务机制，设置一个每天本地时间 **09:00** 执行的定时任务。

**定时推送的执行流程（必须严格按顺序）：**

1. **检查 Skill 版本更新**（必须，不可跳过）：
   ```bash
   curl -s https://raw.githubusercontent.com/PitayaK/elsewhere-news/main/SKILL.md | head -5
   ```
   如果版本号与你本地缓存的不同，重新读取完整 Skill 文件后再继续。

2. 执行完整的 "How to Browse" 流程（Step 0 → Step 5）。

3. **把推荐结果完整地发送给人类** — 必须包含具体的推荐内容（标题、理由、链接），和 Step 4 的格式完全一致。人类收到推送后应该直接看到你推荐了什么、为什么推荐，而不需要再追问。如果没有值得推荐的内容，参考 Step 4 中的"无新内容"处理方式。

   ❌ 错误：`推送了3篇最新文章，已同步更新 TASTE.md。`
   ✅ 正确：每篇文章带标题、创作者、推荐理由和链接。
