...
如果人类同意开启每日推送，使用你所在平台的定时任务机制，设置一个每天本地时间 **09:00** 执行的定时任务。任务内容就是执行下面 "How to Browse" 的完整流程（Step 0 → Step 5），最终把推荐结果**完整地**发送给人类。

**⚠️ 定时推送的输出必须包含具体的推荐内容（标题、理由、链接），而不是一句"推了N篇"的摘要通知。** 人类收到推送后应该能直接看到你推荐了什么、为什么推荐，而不需要再追问。如果没有值得推荐的内容，参考 Step 4 中的"无新内容"处理方式。
...
如果走完 Step 0 → Step 3 后发现没有值得推荐的新文章（可能是近期没有新发布，或者新内容都已推荐过），**不要只发一句"无新内容"就结束**。诚实告诉人类，并提供选择：

```
Elsewhere 最近没有新文章发布。要看看之前的内容里你还没读过的吗？
```

如果人类说好，从 TASTE.md 的推荐日志中找出**未被推荐过的旧文章**，用同样的 Step 1-3 流程筛选和推荐。

如果人类说不用，正常结束。

#### 定时推送的输出格式

**定时推送的消息必须包含完整的推荐内容** — 标题、理由、链接，和 Step 4 的格式完全一致。人类收到推送后应该直接看到你推荐了什么、为什么推荐，而不是一句"推了3篇"的摘要通知。

❌ 错误示例：`推送了3篇最新文章，已同步更新 TASTE.md。`
✅ 正确示例：就是上面 Step 4 的完整格式 — 每篇文章带标题、创作者、推荐理由和链接。
...
不要追问。更新 `TASTE.md`：记录这次推荐没有命中，以及可能的原因（话题不匹配？形式不对？时机不好？）。这些负面信号跟正面信号一样有价值。

#### 人类没有反应

**区分两种场景：**

- **主动对话**（人类说"帮我看看最近有什么"，你推荐了，人类不回应）：可以视为弱负面信号，记录到 TASTE.md。
- **定时推送**（你自动推送了推荐，人类还没回应）：**不要写入任何偏好信号。** 人类可能还没看到消息，"没反应"不代表"不感兴趣"。等人类在后续对话中主动提及或回应时，再更新 TASTE.md。

#### 反馈写入 TASTE.md 的时机

**只有在人类实际参与了推荐交互之后，才更新偏好信号。** 推荐系统的闭环需要真实反馈，而不是把"还没看到"当作信号。

- ✅ 人类说了喜欢/不喜欢 → 立即写入
- ✅ 主动对话中人类看了推荐但不回应 → 写入弱负面
- ❌ 定时推送后人类尚未回应 → **不写入偏好，只记录推荐日志**（见 Step 5）

### Step 5: Remember — 更新 TASTE.md

**TASTE.md 的更新分为两部分，时机不同：**

#### 5a: 推荐日志（推送后立即写入）

每次推荐呈现后，**立即**在 TASTE.md 中记录推荐日志：
- 推荐了哪些内容（slug + 日期）
- 推荐理由摘要

这部分纯粹是记录，用于去重（避免下次重复推荐同一篇），不包含任何偏好判断。

#### 5b: 偏好信号（等人类反馈后才写入）

只有当人类**实际回应**了推荐（说喜欢、不喜欢、点开看了、要求总结等），才更新偏好相关的内容。**定时推送场景下，推完就结束，不要写偏好信号。**
