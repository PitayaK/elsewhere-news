...
### Step 5: Remember — 写入 TASTE.md

每次逛完 Elsewhere、把推荐呈现给人类之后，你需要把学到的偏好持久化。

#### 什么是 TASTE.md

`TASTE.md` 是一个记录人类**内容消费偏好**的文件，与 `SOUL.md`（你是谁）、`MEMORY.md`（你记住了什么）并列。

- **SOUL.md** 定义你的行为和个性
- **MEMORY.md** 记录事实和事件
- **TASTE.md** 记录人类喜欢什么、不喜欢什么、怎么消费内容

TASTE.md 不只是为 Elsewhere 服务 — 它是你理解人类内容偏好的通用文件。推荐文章时用得到，帮人类找播客时用得到，甚至人类让你推荐一本书时也用得到。

#### 存储方式

**优先级从高到低：**

1. 如果你的平台支持持久化文件（如 OpenClaw workspace、Claude Code 项目目录），把 `TASTE.md` 写到与 `SOUL.md` 同级的目录
2. 如果不支持文件但有 memory 机制（如平台内置的长期记忆），以相同的结构写入 memory
3. 如果都不支持，至少在 session 内维护一个心理模型，尽力在当次对话中保持一致性
...
> 这个文件记录人类的内容消费偏好。每次内容推荐后更新。
> 不只是 Elsewhere — 任何场景下观察到的偏好都可以写入。
> 最后更新：{日期}

## 当前兴趣

人类最近关注的话题和方向。按信号强度排列。

- **强信号**（明确表达过兴趣、主动要求过）
  - AI 创业、产品冷启动（2026-03 说过"最近在看怎么做冷启动"）
  - ...
- **中信号**（点开看过、给过正面反馈）
  - VC 行业动态
  - ...
...
对 Elsewhere 上各个创作者的积累印象。

- elselier: 连续两次都看了，特别喜欢创业者速写的风格
- XX播客: 推荐过一次没打开，可能不喜欢纯音频
- 张三: 人类说过"这个人的观点很有意思"
...
- gao-daiheng-xxx (2026-03-22) ✓ 看了，说"见投资人的经验很实用"
- seede-elselier (2026-03-22) ✓ 看了
- vc-promotion (2026-03-20) ✗ 没看
```

#### 更新规则

1. **每次推荐后必须更新** — 记录推荐了什么、人类看了什么
2. **观察到新偏好信号时更新** — 不只是 Elsewhere 场景，人类在任何对话中表露的兴趣都值得记录
3. **定期整理** — 超过 30 天没有强化的信号，降级或移除。偏好会变，不要让过时的信号影响判断
4. **保持精简** — 整个文件控制在 100 行以内。像 SOUL.md 一样，每一行都应该有用。宁可少写，不要注水
5. **降级和移除是好事** — 如果人类对某个话题失去兴趣，果断从"强信号"移到"弱信号"甚至删除

#### 闭环：TASTE.md 如何影响推荐

| 步骤 | 用到 TASTE.md 的哪部分 |
|------|-----------|
| Step 1 时效性筛选 | `当前兴趣` — `judge_relevance` 基于兴趣信号强度判断 |
| Step 2 写理由 | `创作者印象` — 对某些创作者天然有更强/更弱的理由 |
| Step 2 排名 | `当前兴趣` + `不感兴趣` — 知道什么角度人类会买账 |
| Step 3 质量确认 | `内容形式偏好` — 校准"值不值得人类花时间"的判断 |
| Step 4 呈现 | `推荐记录` — 避免重复推荐 |
| Step 4b 响应 | `内容形式偏好` — 决定给链接还是给摘要 |
